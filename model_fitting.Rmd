...
We use dot product of two feature to identify potential subset features. From the result, most feature are sub-group of `windows` which is expected since most games are on windows system. We decide to remove this features since it is not informative to our model. We also found tags `batman`, `fox`, `birds`, `football_american`, `tile_matching`, `tracked_motion_controller_support`only present once, `coding` only appears twice, to make our model more generalizable, we decide to drop these columns.
...
### Lasso regression
We choose lasso regression since it could perform classification and feature selection simutaneously. We uses 5 fold-cv to select appropriate lambda. But before we train our model, we first separate test set and train set to prevent possible data leakage. 
```{r}
...
broom::tidy(lasso_fit) |>
  ...
  labs(x = "Log lambda", y = "Coefficient Estimate", title = "Coefficient estimate at different lambda")
```
...
beta_df|>
  ...
  coord_flip()+
  labs(x = "Weight", y = "Tags", title = "Weight of different tags")
```

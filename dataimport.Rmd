...
## model fitting
Since it is hard for models to directly analyze keywords as string, we one-hot encoded categories, genres, and tags, which facilitate models to analyze the dataset. Also, although most tags are completely descriptive words about the games, some of them might cause data leakage, we filter them out from the dataset. These tags includes: Masterpiece, Great Soundtrack, Addictive. It is easy to observe that they are conclusive words. Also, great_soundtrack, batman, tracked_motion_controller_support, tile_matching have no variance in the dataset, we decide to delete them
```{r}
df_concat = df|>
...
encoded_with_label_df = rbind(popular_encoded_df, unpopular_encoded_df)|>
  janitor::clean_names()|>
  subset(select = -c(positive, negative, estimated_owners, median_playtime_forever, average_playtime_forever, recommendations, user_score, peak_ccu, supported_languages, keywords, addictive, masterpiece, great_soundtrack, batman, tracked_motion_controller_support, tile_matching))|>
...
```{r}
svm_model = svm(train_x_log, train_y, kernel = "linear")
```
...
```{r}
preicted_labels_svm = predict(svm_model, newdata = test_x_log)
```

```{r}
preicted_labels_svm = preicted_labels_svm|>
  data.frame()|>
  mutate(actual_y = test_y)|>
  mutate(different = abs(actual_y != preicted_labels_svm))
```

```{r}
acc_svm = sum(pull(preicted_labels_svm, different))/nrow(preicted_labels_svm)
```

```{r}
confusionMatrix(data=pull(preicted_labels_svm, preicted_labels_svm), reference = pull(preicted_labels_svm, actual_y))
```

```{r}
svm_coef = data.frame(coef(svm_model))

svm_coef_df = data.frame(tags = rownames(svm_coef),
           coef = pull(svm_coef, coef.svm_model.))|>
...
svm_coef_df|>
  subset(tags != "(Intercept)")|>
  head(30)|>
  ggplot(aes(x = reorder(tags, +abs_coef), y = coef))+
  geom_bar(stat="identity")+
  coord_flip()
```

We observed that there are some similar patterns between important feature extracted by Lasso and SVM. 